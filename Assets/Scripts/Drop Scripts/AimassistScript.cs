...
public class AimassistScript : MonoBehaviour
{
    ...
    void OnTriggerStay2D(Collider2D other)
    {
        if (other.tag == "Player")
        {
            other.GetComponent<PlayerController>().aimAssist = true;
            other.GetComponent<PlayerController>().aaB = Time.time + 5;
            other.GetComponent<PlayerController>().aaT = Time.time;
            Destroy(gameObject);
        }
    }
}
